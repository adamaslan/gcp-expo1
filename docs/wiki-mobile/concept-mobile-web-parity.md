...
## Headline: ~62% synced (2026-08-29, after portal PRs #77/#78/#79 + our PR #39 — consent/DSAR land web-only, `attribution.ts` mirrored here)
...
> ⚠️ **Portal PR #77 (2026-08-29) assessed — a cross-surface *obligation* domain lands web-only, plus two portal-only `lib/shared/` modules. Headline drops ~66% → ~63%.** Portal added cookie/tracking-consent infrastructure (`ConsentBanner` + `ConsentPreferences` in its root layout, the `nu_consent` first-party cookie via `POST /api/consent`, `consent_records` + `legal_consent_events` tables), a ToS/Privacy checkbox gating Clerk sign-up, and the data-subject-rights endpoints its privacy policy already promises. Two things move the number. **First**, `lib/shared/consent.ts` and `lib/shared/legal-consent.ts` are new pure modules written to be mirrored here but not yet ported — portal now carries **15** `lib/shared/` modules to our **5**, and they are portable today (consent has no backend/schema dependency we lack). **Second**, consent capture is a genuine cross-surface obligation — GDPR/CPRA bind this app too — so it enters the feature-domain denominator as a real gap: we run `analytics.ts` + `sentry.ts` with **no** consent gate, and the portal is now the only surface that asks. See [[concept-sync-requirements]].

> ⚠️ **Portal PR #78 (2026-08-29) assessed — auth hardening + DSAR rights + analytics/attribution scaffolding, headline ~63% → ~62%.** Builds on PR #77. Three parity-relevant facts. **First**, one new `lib/shared/` module — `attribution.ts`, the pure first-party acquisition model (`nu_attrib` cookie, UTM/gclid/fbclid/referrer, 90-day first-touch), written to be mirrored; our PR #39 does exactly that. **Second**, the auth-hardening work is structurally portal-only and should stay that way: `middleware.ts` edge-matcher coverage and the `lib/http-auth.ts` timing-safe bearer compare guard server routes we do not have. **Third and most consequential**: the data-subject-rights surface (`/api/privacy/{export,profile,delete,rectify}`, the `privacy_requests` statutory-clock ledger) and the analytics consent gate are now web-only. Combined with our un-gated tracking, the shared-identity product has a **second** compliance asymmetry: a user can exercise GDPR access/erasure/rectification on web but has no mechanism here, against the same account — a feature-domain gap, so that denominator moves too.

> ℹ️ **Portal PR #79 (2026-08-29) assessed — portal-only revenue fix + docs, headline unchanged at ~62%.** The portal repaired a live production defect: its `STRIPE_PRICE_ANNUAL` held a placeholder string and no annual price existed on the Stripe account at all, so every annual checkout returned HTTP 500 from a placeholder guard — silently, since monthly worked and nothing alerts on it. Both plans were consolidated onto a single Stripe product, because Stripe's in-place subscription update with proration only works within one product; across two it degrades to cancel-and-resubscribe. The consequent price change ($79 → $79.99) invalidated hardcoded copy in four of their `app/` files, corrected in the same PR. **No `lib/shared/` module touched and none added**, so the single-source denominator holds at 15 portal / 5 here. The feature denominator does not move either: this app has no Stripe checkout surface of its own — billing defers to the portal — so there is no counterpart to drift from. Same portal-only shape as their #59/#64/#65/#67/#70. The portable content is documentary rather than code, and worth knowing on this side too: Stripe prices can never be deleted (only archived, `active:false`), archiving does **not** migrate existing subscribers off a price, and `vercel env add` silently refuses to overwrite an existing variable — all three bite any surface that touches the same Stripe account or Vercel project. See `nuwrrrld-portal/docs/stripe-price-consolidation.md`.
