...
| ~~`lib/shared/signal-policy.ts`~~ | **Done — our PR #32 (2026-08-08); re-synced our PR #43 (2026-09-04).** Adopted verbatim (pure ticker validation / cache-freshness / backoff) before we grew our own copy. Portal PR #101 added `isCryptoShaped()` to the shared module the same week; our PR #43 ported it same-day to restore byte-identity. Tracked by the drift gate. Not consumed by a feature here yet — see §2's Signal cache/queue row. |
...
| ~~`lib/shared/attribution.ts`~~ | **Adopted — our PR #39 (2026-09-01).** Mirrored verbatim from portal `main` (portal PR #81): pure first-party acquisition model (`nu_attrib` cookie, UTM/gclid/fbclid/referrer, 90-day first-touch) with a `parseTouch` hardened against a tampered cookie (`cleanKnownKeys` filters to known string keys; `isValidTimestamp` guards `ts`). Byte-identical, `normalize: null` in the drift gate on both repos. Portal already had it on `main`, so no circular-gate stall. **Still owed:** a mobile attribution-capture path (§2) — the model is here, nothing calls it. |
| `lib/shared/consent.ts`, `lib/shared/legal-consent.ts` | New on the portal (PRs #77/#78), mobile-absent. Pure, written to be mirrored — no backend/schema dependency we lack, only the `prefs.ts` storage seam already solved. **Priority #1** (see below): GDPR/CPRA bind this app too, and `analytics.ts` + `sentry.ts` currently run with no consent gate. |
...
6. ~~Adopt `lib/shared/attribution.ts` before we reimplement it.~~ — **done, our PR #39 (2026-09-01).** Byte-identical, drift-gated. Not consumed yet — see #8.
7. **Adopt `lib/shared/consent.ts` + `legal-consent.ts` and gate `analytics.ts` / `sentry.ts` behind them** (portal PRs #77/#78). Compliance obligation, not a nicety — a user can manage tracking consent on web but not here, on the same account. Portable now (only the `prefs.ts` storage seam).
8. Build a mobile attribution-capture path that consumes the now-adopted `lib/shared/attribution.ts` — POST landing query params + referrer to a mobile endpoint on first authenticated load.
9. Build a mobile DSAR path (export / delete / rectify) — portal PR #78 shipped these web-only; the same account has no mechanism here.
10. Record the AI Council convergence decision.
11. Port observability (analytics/Sentry) to portal; port backtest to mobile (or decide against). Wire ourselves up to consume `signal-policy.ts`/`live-price.ts` if the real-time signal tier is ever ported.
