...
## [2026-08-29] ingest | Portal PRs #77/#78 + our PR #39 (feat(shared): mirror attribution.ts + register in drift guard) | pages touched: 3

Caught the parity pair up across three portal-side events. Portal PR #77 landed cookie/tracking-consent infrastructure web-only (`ConsentBanner`/`ConsentPreferences`, `nu_consent` cookie, `/api/consent`, `/api/legal-consent`, `/api/privacy/{export,profile,delete}`, `consent_records`+`legal_consent_events`) plus two portal-only `lib/shared/` modules (`consent.ts`, `legal-consent.ts`) — a genuine cross-surface obligation (GDPR/CPRA bind this app too, and we track with no consent gate), so both denominators moved: ~66% → ~63%. Portal PR #78 added auth hardening (`middleware.ts` matcher, pure-JS `lib/http-auth.ts` timing-safe compare — structurally portal-only), the DSAR rights surface + `privacy_requests` statutory-clock ledger, the `lib/analytics.ts` consent gate (all web-only), and one new mirror-intended `lib/shared/` module, `attribution.ts`: ~63% → ~62%. Our PR #39 then mirrored `attribution.ts` verbatim (pure first-party acquisition model — `nu_attrib` cookie, UTM/gclid/fbclid/referrer, 90-day first-touch) and registered it in `scripts/check-shared-drift.mjs` on both repos (`normalize: null`). We now carry 6 of portal's 16 `lib/shared/` modules — single-source parity ~38% → ~40% — but no consumer wiring here yet (a mobile attribution-capture path is still owed). Feature-domain parity and the blended headline hold at ~62%. New sync-requirements priorities: adopt `consent.ts` + gate our observability (#1), build attribution capture (#2), build a mobile DSAR path (#3). Pairs with portal PR #78.

## [2026-08-29] ingest | portal PR #79 fix(stripe): repair broken annual checkout, consolidate onto one product | pages touched: 2
